/*************************************************************************
	...
	> Created Time: 2018年04月11日 星期三 09时54分15秒
 ************************************************************************/
...
//月份简写
const string month_abbr[] = {"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sept","Oct","Nov","Dec"};		//string类型数组是二维的
...
date::date(string &ds)
{
	...
	if (p > 0)	{		//月份名格式
		...
		p = q;
		if(month < 1 || month > 12)
			throw invalid_argument("不是月份合法值");
		if(ds[p++] != '/')
			throw invalid_argument("非法间隔符");
		day = get_day(ds,month,p);
		if(ds[p++] != '/')
